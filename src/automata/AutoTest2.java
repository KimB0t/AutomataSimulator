 ...
 * @version 1.0
 */
public class AutoTest2 extends Automaton{
    
    //<editor-fold defaultstate="collapsed" desc="Declarations">
    private CellTest2[][] matrix;
//</editor-fold>
    
    //<editor-fold defaultstate="collapsed" desc="Constructors">
    public AutoTest2() {
        super();
        this.matrix = new CellTest2[MATRIX_LENGTH][MATRIX_LENGTH];
    }
//</editor-fold>
...
//</editor-fold>
    ...
    public void makeWallAt(int i, int j) {
        this.matrix[i][j] = new CellTest2(getCOLOR_OBSTACLE(), i, j, true);
    }
    
    @Override
    public void makeCellAt(int i, int j) {
        this.matrix[i][j] = new CellTest2(getParams(), 0, getCOLOR_DEFAULT(), i, j, false);
    }
//</editor-fold>
    ...
    public void step() {
        ...
        CellTest2[][] new_matrix_class = 
                new CellTest2[MATRIX_LENGTH][MATRIX_LENGTH];
        ...
        for(int i=0; i<MATRIX_LENGTH; i++) {
            for(int j=0; j<MATRIX_LENGTH; j++){
                ...
                CellTest2[] listNghbrs = getListOfNeighbours(i, j, 8);
        ...
        for(int i=0; i<MATRIX_LENGTH; i++) {
            for(int j=0; j<MATRIX_LENGTH; j++){
                ...
                int di = (int)new_matrix_class[i][j].getDelta().getX();
                int dj = (int)new_matrix_class[i][j].getDelta().getY();
                
                if (di != -1 && dj != -1){
                    //moving agents
                    new_matrix_class[i][j].decreaseNbAgents(1);
                    new_matrix_class[di][dj].MoveAgent(getParams(), new_matrix_class[i][j].getClasse());
                }
...
//                new_matrix_class[i][j].printCell("new_matrix_cell final step == ");
    ...
    @Override
    public CellTest2[] getListOfNeighbours(int i, int j, int nbNghbrs) {
        
        CellTest2[] nb = new CellTest2[nbNghbrs];
        int m = 0;
